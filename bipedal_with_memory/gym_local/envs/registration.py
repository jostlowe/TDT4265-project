    ...
    def spec(self, path):
        if ':' in path:
            ...
            # catch ImportError for python2.7 compatibility
                ...
                raise error.Error('A module ({}) was specified for the environment but was not found, make sure the package is installed with `pip install` before calling `gym_local.make()`'.format(mod_name))
        else:
            id = path
